...
while true
    ...
    if toc(last_check_time) >= check_interval
        ...
        % ---------- 固定中继通信指标（UAV1 和 UGV1 在原材料区） ----------
        % 转换无人机坐标到世界坐标系
        uav1_world_pos = [latest_uav_pos(2), latest_uav_pos(1), -latest_uav_pos(3)];
        uav2_pos = [15, 0, 2];                     % 基站位置（世界坐标）
        
        if robots{1}.has_pose
            ...
            [d1, PL1_dB, Pr1_dBm, SINR1_dB, BER1, ...
             ...
             SINR_eq_dB, BER_eq, delay_s] = compute_fixed_relay_metrics(uav1_world_pos, ugv1_pos, uav2_pos);
            ...
            save_fixed_relay_data_to_excel(data_log);
        else
            fprintf('【固定中继】UGV1 位置尚未获取，跳过计算\n');
        end
        % ----------------------------------------------------------------
...
% ==================== 固定中继通信指标计算（原材料区固定） ====================
% ------------------------------------------------------------------------
% 函数名称: compute_fixed_relay_metrics
% 功能描述: 固定中继场景（UAV1 → UGV1 → UAV2），UAV1 和 UGV1 均在原材料区。
%           返回第一跳和第二跳的所有指标以及端到端等效指标，同时打印到终端。
% 输入参数:
%   uav1_pos   - UAV1 世界坐标 [x, y, z]
%   ugv1_pos   - UGV1 世界坐标 [x, y, 0]
%   uav2_pos   - UAV2 世界坐标 [15, 0, 10]
% 输出参数:
%   d1, PL1_dB, Pr1_dBm, SINR1_dB, BER1  - 第一跳指标
%   d2, PL2_dB, Pr2_dBm, SINR2_dB, BER2  - 第二跳指标
%   SINR_eq_dB, BER_eq, delay_s           - 端到端等效指标
% ------------------------------------------------------------------------
function [d1, PL1_dB, Pr1_dBm, SINR1_dB, BER1, ...
          d2, PL2_dB, Pr2_dBm, SINR2_dB, BER2, ...
          SINR_eq_dB, BER_eq, delay_s] = compute_fixed_relay_metrics(uav1_pos, ugv1_pos, uav2_pos)
    % 固定系统参数
    ...
    if any(isnan(ugv1_pos)) || any(isinf(ugv1_pos))
        error('UGV1 位置无效');
    end
    ...
    [SINR1_lin, SINR1_dB, Pr1_dBm, PL1_dB, ~, d1] = ...
        calc_single_link_deterministic(uav1_pos, ugv1_pos, Pt_uav_dBm, PL0_dB, n_eff1, N0_1_dBm);
    BER1 = 0.5 * exp(-SINR1_lin);

    % 第二跳（也使用原材料区参数，独立随机，但衰落确定性）
    n2 = 2.5 + 0.5 * rand();
    noise_rise2 = 6 + 3 * rand();
    N0_2_dBm = -100 + noise_rise2;
    m2 = 1.5 - 0.75 * rand();   % 保留但不再使用
    if is_blocked
        n_eff2 = n2 + 0.5;
    else
        n_eff2 = n2;
    ...
    [SINR2_lin, SINR2_dB, Pr2_dBm, PL2_dB, ~, d2] = ...
        calc_single_link_deterministic(ugv1_pos, uav2_pos, Pt_ugv_dBm, PL0_dB, n_eff2, N0_2_dBm);
    BER2 = 0.5 * exp(-SINR2_lin);

    % 端到端等效指标（解码转发，取最小 SINR）
    ...
    % 打印到终端
    fprintf('【固定中继通信指标】(UAV1 和 UGV1 均在原材料区)\n');
    fprintf('=== 第一跳 (UAV1 → UGV1) ===\n');
    fprintf('  距离: %.2f m | 路径损耗: %.2f dB | 接收功率: %.2f dBm | SINR: %.2f dB | BER: %.2e\n', ...
            d1, PL1_dB, Pr1_dBm, SINR1_dB, BER1);
    fprintf('=== 第二跳 (UGV1 → UAV2) ===\n');
    fprintf('  距离: %.2f m | 路径损耗: %.2f dB | 接收功率: %.2f dBm | SINR: %.2f dB | BER: %.2e\n', ...
            d2, PL2_dB, Pr2_dBm, SINR2_dB, BER2);
    fprintf('=== 端到端等效 ===\n');
...
% ------------------------------------------------------------------------
% 函数名称: save_fixed_relay_data_to_excel
% 功能描述: 将记录的数据写入 Excel 文件（覆盖写入）
% 输入参数: data_log - cell 数组，每行包含所有记录指标
% ------------------------------------------------------------------------
function save_fixed_relay_data_to_excel(data_log)
    if isempty(data_log)
        return;
    end
    % 定义列名
    ...
    filename = 'fixed_relay_data.xlsx';
    try
