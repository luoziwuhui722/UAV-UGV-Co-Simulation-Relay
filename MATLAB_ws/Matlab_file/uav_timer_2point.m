...
publish_rate = 0.1;          % 100 ms 发布周期
check_interval = 0.5;        % 500 ms 状态检查周期
setpoint_counter = 0;
...
% 状态机定义
% state: 'initial_climb' -> 初始爬升到 (0,0,-6)
%        'going_to_1'    -> 飞向目标点1
%        'going_to_2'    -> 飞向目标点2
%        'waiting'       -> 停留中
%        'finished'      -> 已完成10次来回，停止移动
state = 'initial_climb';           % 初始状态：先爬升到中间点
wait_start_time = 0;            % 停留开始时间（Unix 秒）
round_trip_count = 0;           % 已完成的来回次数（一次来回 = 1->2->1）

% 目标点坐标
initial_target = [0.0, 0.0, -9.0];  % 新增：初始爬升点（先飞高）
target1 = [-5.0, -10.0, -5.0];
target2 = [5.0, -20.0, -5.0];
current_target = initial_target;   % 当前目标点（用于发布）
...
while true
    ...
    % 根据状态选择当前目标点
    if strcmp(state, 'initial_climb')
        current_target = initial_target;
    elseif strcmp(state, 'going_to_1')
    ...
    if toc(last_check_time) >= check_interval
        ...
        if ~strcmp(state, 'finished') && ~any(isnan(latest_pos))
            % 初始爬升状态到达检测
            if strcmp(state, 'initial_climb')
                dx = abs(latest_pos(1) - initial_target(1));
                dy = abs(latest_pos(2) - initial_target(2));
                dz = abs(latest_pos(3) - initial_target(3));
